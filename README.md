...
## Краткий сценарий демонстрации проекта

1. Запустить проект командой `docker compose up --build`.
2. Открыть главную страницу и показать витринную ленту публикаций.
3. Открыть любую карточку и показать модалку публикации.
4. Нажать на автора публикации и открыть все посты этого пользователя.
5. Зарегистрировать нового пользователя и получить JWT на backend.
6. Создать свою публикацию через интерфейс.
7. Открыть раздел со своими публикациями и показать, что новая карточка появилась в ленте пользователя.
8. Изменить и удалить свою публикацию.
9. Показать `GET /api/v1/posts/user/{user_id}` два раза подряд в Postman или Swagger, чтобы продемонстрировать работу Redis-кэша.
10. Коротко показать файлы `dependencies/services.py` и `dependencies/auth.py`, чтобы объяснить использование DI.
